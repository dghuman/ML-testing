...
def hit_array(empty_array, _track):
    ...
    for n in range(dim):
        ...
        for m in range(dim):                                               # Loop over values y bins
            cond = np.logical_and(np.less_equal(yval, bin_array[m+1]),np.greater_equal(yval, bin_array[m]))    # Check if yval is in bin
...
def main():
    ...
    for i in range(100):                        # Make 100 entries
        ...
        data_set["slope"].append(m)
        data_set["intercept"].append(b)
        data_set["arr"].append(hits)
    save_pickle(data_set, '/home/users/ghuman/ML/DataSetGen/2D_data/data_1.pkl')
